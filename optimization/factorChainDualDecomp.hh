...
public:
  ...
  //DON'T PUBLISH -- START
  //returns a logarithmic offset, i.e. logarithm of a common factor
  //this variant is numerically heavily unstable. use logspace routine instead!
  virtual double compute_sum_forward(const ChainDDVar* incoming, const ChainDDVar* outgoing, const Math1D::Vector<double>& prev_forward, Math1D::Vector<double>& forward,
                                     double mu) const noexcept;

  virtual double compute_sum_forward_logspace(const ChainDDVar* in_var, const ChainDDVar* out_var, const Math1D::Vector<double>& prev_log_forward, 
                                              Math1D::Vector<double>& log_forward, double mu) const noexcept;

  //this variant is numerically heavily unstable. use logspace routines instead!
  virtual void compute_marginals(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
  ...
  virtual void compute_marginals_logspace(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                                          const Math1D::Vector<double>& log_forward1, const Math1D::Vector<double>& log_forward2,
                                          double mu, Math1D::Vector<double>& marginal) const noexcept;

  virtual void compute_all_marginals_logspace(const ChainDDVar* in_var1, const ChainDDVar* in_var2, const Math1D::Vector<double>& log_forward1, 
                                              const Math1D::Vector<double>& log_forward2, double mu, Storage1D<Math1D::Vector<double> >& marginal) const noexcept;
  //DON'T PUBLISH -- END
...
public:
  ...
  //DON'T PUBLISH -- START
  double compute_sum_forward(const ChainDDVar* incoming, const ChainDDVar* outgoing,
                             const Math1D::Vector<double>& prev_forward, Math1D::Vector<double>& forward,
                             double mu, const Math2D::Matrix<float>& cost) const noexcept;

  double compute_sum_forward_logspace(const ChainDDVar* incoming, const ChainDDVar* outgoing,
                                      const Math1D::Vector<double>& prev_log_forward, Math1D::Vector<double>& log_forward,
                                      double mu, const Math2D::Matrix<float>& cost) const noexcept;


  void compute_marginals(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                         const Math1D::Vector<double>& forward1, const Math1D::Vector<double>& forward2,
                         double mu, const Math2D::Matrix<float>& cost, Math1D::Vector<double>& marginal) const noexcept;

  void compute_marginals_logspace(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                                  const Math1D::Vector<double>& log_forward1, const Math1D::Vector<double>& log_forward2,
                                  double mu, const Math2D::Matrix<float>& cost, Math1D::Vector<double>& marginal) const noexcept;

  void compute_all_marginals_logspace(const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                                      const Math1D::Vector<double>& log_forward1, const Math1D::Vector<double>& log_forward2,
...
public:
  ...
  //DON'T PUBLISH -- START
  virtual double compute_sum_forward(const ChainDDVar* incoming, const ChainDDVar* outgoing, const Math1D::Vector<double>& prev_forward, Math1D::Vector<double>& forward,
                                     double mu) const noexcept override;

  virtual double compute_sum_forward_logspace(const ChainDDVar* incoming, const ChainDDVar* outgoing, const Math1D::Vector<double>& prev_log_forward, 
                                              Math1D::Vector<double>& log_forward, double mu) const noexcept override;


  virtual void compute_marginals(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                                 const Math1D::Vector<double>& forward1, const Math1D::Vector<double>& forward2,
                                 double mu, Math1D::Vector<double>& marginal) const noexcept override;
  ...
  virtual void compute_all_marginals_logspace(const ChainDDVar* in_var1, const ChainDDVar* in_var2, const Math1D::Vector<double>& log_forward1, 
                                              const Math1D::Vector<double>& log_forward2, double mu, Storage1D<Math1D::Vector<double> >& marginal) const noexcept override;
  //DON'T PUBLISH - END
...
public:
  ...
  //DON'T PUBLISH - START
  virtual double compute_sum_forward(const ChainDDVar* incoming, const ChainDDVar* outgoing,
                                     const Math1D::Vector<double>& prev_forward, Math1D::Vector<double>& forward,
                                     double mu) const noexcept override;
...
public:
  ...
  //DON'T PUBLISH - START
  double compute_sum_forward(const ChainDDVar* incoming, const ChainDDVar* outgoing, const Math1D::Vector<double>& prev_forward, Math1D::Vector<double>& forward,
                             double mu, const Math3D::Tensor<float>& cost) const noexcept;

  double compute_sum_forward_logspace(const ChainDDVar* in_var, const ChainDDVar* out_var, const Math1D::Vector<double>& prev_log_forward, Math1D::Vector<double>& log_forward,
                                      double mu, const Math3D::Tensor<float>& cost) const noexcept;


  void compute_marginals(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                         const Math1D::Vector<double>& forward1, const Math1D::Vector<double>& forward2,
                         double mu, const Math3D::Tensor<float>& cost, Math1D::Vector<double>& marginals) const noexcept;
...
public:
  ...
  virtual void compute_marginals(const ChainDDVar* target, const ChainDDVar* in_var1, const ChainDDVar* in_var2,
                                 const Math1D::Vector<double>& forward1, const Math1D::Vector<double>& forward2,
                                 double mu, Math1D::Vector<double>& marginals) const noexcept override ;
...
public:
  ...
  double compute_sum_forward_logspace(const ChainDDVar* in_var, const ChainDDVar* out_var,
                                      const Math1D::Vector<double>& prev_log_forward, Math1D::Vector<double>& log_forward,
                                      double mu, const Storage1D<Math3D::Tensor<float> >& cost) const noexcept;
...
public:
  ...
  double smooth_optimization_logspace(uint nIter, double epsilon = 1.0);

  double smooth_optimization_LBFGS(uint nIter, double epsilon = 1.0, uint L=5, SDD_LBFGS_MODE mode = SDD_LBFGS_IMPLICIT_CONSTRAINTS);
...
protected:
  ...
  //!!!DON'T PUBLISH
  void multiply_with_lbfgs_matrix(const Storage1D< Storage1D<Storage1D<Math1D::Vector<double> > > >& step,
                                  const Storage1D< Storage1D<Storage1D<Math1D::Vector<double> > > >& grad_diff,
                                  const Math1D::Vector<double>& rho, double initial_scale, int iter, int last_restart,
                                  Storage1D<Storage1D<Math1D::Vector<double> > >& vec) const;
