...
public:
  ...
  void update_duals(DualBCAMode mode, const Math1D::Vector<float>& card_cost, const Math1D::Vector<uint>& level);

  double dual_value(const Math1D::Vector<float>& card_cost, const Math1D::Vector<uint>& level) const;
